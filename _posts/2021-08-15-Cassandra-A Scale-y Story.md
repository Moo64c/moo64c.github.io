...
![Application machine with Communicator vs. application machine without communicator. Number of connections to Cassandra is constant with Communicator.](../assets/images/2021-08-15-Cassandra-A%20Scale-y%20Story/2.png)
...
![Graph of communicator connections and Cassandra Connections. Shows over 3200 communicator clients and 320 Cassandra connections](../assets/images/2021-08-15-Cassandra-A%20Scale-y%20Story/1.png)
...
![Application response time before and after Communicator. Percentage of requests with response time over 0.5 second drops from 15% to 2% with Communicator](../assets/images/2021-08-15-Cassandra-A%20Scale-y%20Story/3.png)
...
What we have left to gain here is by further reducing stress on the Cassandra side will probably come with a performance cost to the application side - such as moving CC out of the machines themselves to separate machines, maybe one or two per cluster. You might have a single database connection for a dozen machines! On one side, fewer connections to the database itself; on the other side - the application will have to jump through more hoops to send requests and receive responses, making that "less than 1 millisecond" baseline a thing of the past. We halted current CC optimizations without taking it out of the application machines, although it was in the original design (and that is why we used TCP sockets and not Unix sockets).
