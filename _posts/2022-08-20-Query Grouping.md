...
The only obvious limits to use `query_grouping` is that the callbacks added do not directly depend on one another. You could share state to some degree and designed carefully - even have some results from one callback used in a different one. There are no guarantees on order of execution inside a Group except the yields between queries match. This means you should also avoid using *thread-unsafe* stateful code like a Redis pipeline in a coroutine.
